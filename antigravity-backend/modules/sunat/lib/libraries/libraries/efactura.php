...
require_once __DIR__ . '/see/lib/xmlseclibs-master/xmlseclibs.php';
class Factura
{
...
//		$publicKey = file_get_contents('prueba/server.pem');
		...
		$privateKey = file_get_contents(__DIR__ . "/certificado_digital/$modo/server_key.pem");
		$publicKey = file_get_contents(__DIR__ . "/certificado_digital/$modo/server.pem");
...
//		$publicKey  = file_get_contents("server.pem");
		...
		$objSign = new XMLSecurityDSig('Sign' . $ruc);
		...
		if (!($Node))
			$Node = $domDocument->getElementsByTagName($ReferenceNodeName)->item(0);
