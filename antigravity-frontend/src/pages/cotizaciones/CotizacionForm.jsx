        ...
        try {
            ...
            if (finalClienteId === 'NEW' && selectedCliente) {
                if (!selectedCliente.razon_social || !selectedCliente.numero_documento) {
                    setLoading(false);
                    return toast.error('Faltan datos obligatorios del cliente');
                }
                const resCli = await apiClient.post('/clientes', {
                ...
                if (resCli.success && resCli.data?.id) {
                    ...
                    if (match) {
                        ...
                        setLoading(false);
                        return toast.error('Error al registrar cliente');
                    }
                }
            }

            const res = cotizacion
                ? await cotizacionesApi.actualizar(cotizacion.id, { ...payload, cliente_id: finalClienteId })
                : await cotizacionesApi.crear({ ...payload, cliente_id: finalClienteId });
