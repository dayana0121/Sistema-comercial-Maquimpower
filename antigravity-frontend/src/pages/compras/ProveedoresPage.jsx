    ...
    <div className="p-6 max-w-[1400px] mx-auto">
        ...
        : filtrados.length === 0
          ...
          : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtrados.map((p, index) => {
              ...
              return (
                <div key={p.id} className={`bg-white rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-shadow flex flex-col relative overflow-hidden ${borderColor}`} style={{ borderTopWidth: '4px' }}>
                  ...
                  <div className="p-5 flex-1">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <div className="flex gap-1 shrink-0 bg-slate-100 rounded-md p-0.5">
                        <button onClick={() => abrirEditar(p)} className="p-1.5 text-slate-500 hover:text-slate-800 hover:bg-white rounded transition-colors"><Edit size={14} strokeWidth={2.5} /></button>
                        <button onClick={() => handleEliminar(p.id)} className="p-1.5 text-slate-500 hover:text-red-600 hover:bg-white rounded transition-colors"><Trash2 size={14} strokeWidth={2.5} /></button>
                      </div>
                    </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}
        ...
        <div className="bg-white">
          {!editando && (
            <div className="mb-8 bg-slate-50 p-6 rounded-xl border border-dashed border-slate-200">
              <BuscadorDocumento
            ...
            {[
              ...
              <div key={f.name} className="form-group-custom">
                ...
                <div className="form-input-container">
                  {f.icon && <div className={`form-input-icon ${f.color}`}><f.icon size={16} strokeWidth={2.5}/></div>}
        ...
        <div className="form-actions-custom border-t text-sm border-slate-100 flex justify-end gap-3 mt-4">
